...
def item_purchase_params
  params.require(:item_purchase_address).permit(:postal_code, :prefecture_id, :city, :address, :phone_number, :item_purchase, :building_name).merge(user_id: current_user.id, item_id: params[:item_id], token: params[:token])
end
